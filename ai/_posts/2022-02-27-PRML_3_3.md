...
1. 우선 결합 밀도 $p(x, t)$를 구하는 추론 문제를 풀어낸다. 다음에 이를 정규화하여 조건부 밀도 $p(t|x)$를 구하고 최종적으로 식 $\int tp(t|\mathbf{x}) dt$ 의 조건부 평균을 구한다
2. 우선 조건부 밀도 $p(x|\mathbf{x})$를 구하는 추론 문제를 풀고 $\int tp(t|\mathbf{x})dt$ 의 조건부 평균을 구한다.
